		...
		local function ApplyAnimationSet(setData)
		...
		print("ApplyAnimationSet berjalan")

	end

	-------------------------------------------------
	-- APPLY SATU ANIMASI (LAMA)
	-------------------------------------------------
	local function ApplyInstantAnimation(category, data)

		if category == "Set" then
			ApplyAnimationSet(data)
			return
		end
		...
		-- Urutan Tampilan
		local CategoryOrder = {"Set", "Idle", "Walk", "Run", "Jump", "Fall", "Climb","SwimIdle", "Swim"}
